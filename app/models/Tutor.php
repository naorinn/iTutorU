...
class Tutor extends Model 
{
	...
	public function getTutors($subject = '', $program='', $price = 0, $price_upper = 0){
		$sql = "";
		/*if($program != '' || $price != ''){
			$sql = "SELECT t.userId, p.firstName, p.lastName, p.profileImagePath, t.pay, t.description, t.timesTutored, t.rating FROM Tutor t, Profile p WHERE t.userId=p.userId AND t.description LIKE :subject OR p.programId = :program OR (t.pay BETWEEN :price AND :price_upper)";
			$subject = "%$subject%";
			$stmt = self::$_connection->prepare($sql);
